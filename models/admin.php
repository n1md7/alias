...
class AdminModel extends Model{
	...
	public function wordsEdit(){
		...
		switch (true):
			...
			case $post['action'] == 'wordAdd' && isDefined($post['cat_id']) && isDefined($post['word']) && isDefined($post['lang']):
				$this->query('INSERT INTO words (cat_id, word, lang) VALUES (:cat_id, :word, :lang)');
				$this->bind(':cat_id', $post['cat_id']);
				$this->bind(':word', $post['word']);
				$this->bind(':lang', $post['lang']);
				$this->execute();
				if($this->lastInsertId()){
					exit(json_encode([
						'Success'=>'A Word has been saved successfully!',
						'id'=>$this->lastInsertId()
					]));
				}else{
					exit(json_encode(['Error'=>'Unsuccessful try!']));
